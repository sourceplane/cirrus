...
data "external" "adopt_rate_limit_namespace" {
  program = ["bash", "-c", <<-EOT
    set -euo pipefail
    title="$(jq -r .title)"
    state_file="$(mktemp)"
    ...
    if [ "$code" = "200" ] && jq -e \
        '[.resources[]? | select(.type == "cloudflare_workers_kv_namespace" and .name == "rate_limit")] | length > 0' \
...
import {
  for_each = toset(compact([data.external.adopt_rate_limit_namespace.result.id]))
