...
| [`saas-baseline-tracking/`](./saas-baseline-tracking/) | **BT** | Draft (plan; BT0 shipped in orun-cloud) | flows/common, flows/phases/*, flows/agent; orun-cloud packages/mcp | The bootstrap tracks itself in the workspace's task plane: epic `infra-baselining`, one milestone per phase, one task per landing, PR branches `orun/BASE-n-<phase>` so every landing binds and folds to done from observed evidence. BT0 (MCP `epic_create` / `milestone_create` / widened `task_create`) shipped. |
