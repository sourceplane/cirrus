...
| [`saas-baseline-tracking/`](./saas-baseline-tracking/) | **BT** (cross-repo umbrella: BT0 orun-cloud · BT-O1–O4 orun · BT1–BT6 here) | In progress (BT0, BT-O1–O4, BT1–BT5 ✅; BT6 partial — rehearsal gated) | flows/common, flows/phases/*, flows/agent; orun `cmd/orun`, `internal/platformmcp`; orun-cloud `packages/mcp` | The bootstrap tracks itself in the workspace's task plane: epic `infra-baselining`, one milestone per phase, one task per landing, PR branches `orun/BASE-n-<phase>` so every landing binds and folds to done from observed evidence. BT0 (MCP `epic_create` / `milestone_create` / widened `task_create`) shipped. |
...
| [`saas-bootstrap-console/`](./saas-bootstrap-console/) | **BC** (cross-repo: BC1–BC3 here, lumen, stratus · BC-K1–BC-K5 orun-cloud) | Draft (not started) | `blueprint.yaml`, `flows/testing`, `tests/flows`; orun-cloud `agents-worker`, `web-console-next` | The manifest is the contract, the console is the surface. `blueprint.yaml` gains `overview`/`secrets`/`programme` and loses `spec.source.tag` (which read `baseline-v1` against a live `baseline-v4` for three releases, unnoticed); orun-cloud's bootstrap door serves it, and the console renders a six-step flow, a live build page and a resumable draft entirely from it. Secrets and programme are previews — `create-secrets.sh` and `track.sh` keep writing them. |
| [`saas-bootstrap-engine/`](./saas-bootstrap-engine/) | **BE** (cross-repo: BE1–BE6 here · BE-K1–BE-K4 orun-cloud · BE-O1–BE-O8 orun) | Draft (not started) | `repo-blueprint.yaml` (absorbs all of `flows/`), `tooling/blueprint` + `tooling/bootstrap` (retired), `flows/testing/manifest.test.sh`, `.github/workflows/ci.yml` | **The blueprint is the bootstrap.** `flows/` is 4,971 lines expressing one contract eight times — the same baseline-fetch preamble appears 59 times, `ctx.sh` is sourced 32 — wrapped around eleven shell scripts that are thin policy over verbs orun already has. BE moves the mechanism into the binary as **typed actions** (`uses: orun.pr/land@v1`, not `run: ["orun", …]`), promotes `phases:` to orun's native `Phase` overlay with its own hooks and barrier (retiring `split-phases.py` and its cross-phase edge prune), and leaves this repo declaring only what is its own: what to place, in what order, what it needs first, and **pre-authored narration** printed from engine state rather than paraphrased by an agent. Phase state is **derived, never stored** — which is why a phase can still be run alone, months later, from a fresh container. Adds the CI ladder this repo has never had: static gates, dry instantiation with a **leak gate** (today every product ships this repo's `ai/context/` provenance, which `BOOTSTRAP.md` promises it does not), phase simulation with resume determinism, and a **live bootstrap required before any `baseline-vN` tag**. |
