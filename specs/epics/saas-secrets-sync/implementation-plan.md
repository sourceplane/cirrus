...
- Define the escrow document: one AWS Secrets Manager secret per environment
  at `sourceplane/cirrus/worker-secrets/<env>`, JSON shape:
  ...
  the same mechanism and IAM grant (`sourceplane/cirrus/*` is already in
...
- Human writes the live values into
  `sourceplane/cirrus/worker-secrets/{stage,prod}`:
