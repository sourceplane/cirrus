...
|------|-------------------|----------------|
| **SS3 — escrow seeding** | Human must write live secret values into `sourceplane/cirrus/worker-secrets/{stage,prod}`. The existing manually-`put` values (notably `SECRET_ENCRYPTION_KEY` — it encrypts data at rest) must be escrowed *as-is*, not regenerated. | Values present; SS1 escrow check green against live Secrets Manager. |
