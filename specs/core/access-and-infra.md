...
- `SUPABASE_API_KEY` as a GitHub Actions secret with management access to the
  Supabase `cirrus` organization. Terraform jobs must map this secret to
...
- Organization/account name: `cirrus`
...
- Supabase `stage` and `prod` projects are separate, Terraform-created projects
  under organization `cirrus` (`dwazxcrywsdbxpuouifa`), and their generated
