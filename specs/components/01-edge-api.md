...
|----------------|-------------------------|
| stage | `https://${CONSOLE_CUSTOM_DOMAIN}` (`https://stage.cirrus.app`), `https://cirrus-web-console-next-stage.<workers-dev-subdomain>.workers.dev`, localhost |
| prod | `https://${CONSOLE_CUSTOM_DOMAIN}` (`https://prod.cirrus.app`), `https://cirrus-web-console-next-prod.<workers-dev-subdomain>.workers.dev`, localhost |
...
Legacy `cirrus-web-console-{env}.pages.dev` Pages origins were removed
...
`prod.cirrus.app` calling stage API) are rejected. Custom domain origins
