...
This is the **cross-epic index** for the Cirrus SaaS starter. It groups the
...
| **BT** | [`epics/saas-baseline-tracking/`](./epics/saas-baseline-tracking/) | In progress (all but the BT6 rehearsal ✅) | The bootstrap as tracked work, across three repos: BT0 MCP container writes (orun-cloud, ✅) · BT-O1–O4 `orun task --epic/--contract`, `orun task epic|milestone`, the 33-tool manifest in `orun mcp serve`, `orun pr open --branch-slug` (orun) · BT1 `track.sh` over `orun task` (idempotent, degrades loudly) · BT2 `land-pr.sh` on `orun/<KEY>-<phase>` + trailer + manifest · BT3 phases ensure milestone + task, umbrella ensures the epic · BT4 phase 01 lands as a PR · BT5 the agent lays out the programme and reports the rollup · BT6 rehearsal + contract test. |
| **BC** | [`epics/saas-bootstrap-console/`](./epics/saas-bootstrap-console/) | Draft (not started) | The manifest is the contract, the console is the surface: `blueprint.yaml` v2 (`overview`, `secrets`, `programme`; `spec.source.tag` deleted) + a conformance test against the flows that do the work (BC1–BC2), ported to lumen and stratus (BC3); orun-cloud serves the manifest and computes readiness from it (BC-K1), then a six-step bootstrap flow (BC-K2), a live build page whose work plan *is* the progress view (BC-K3), a workspace-side resumable draft with an Overview card (BC-K4) and a bootstrap contract that ends the three-`--set`-flags handoff (BC-K5). Pixel target is a published reference UI. |
