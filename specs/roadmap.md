...
| **BT** | [`epics/saas-baseline-tracking/`](./epics/saas-baseline-tracking/) | Draft (plan; BT0 shipped) | The bootstrap as tracked work, across three repos: BT0 MCP container writes (orun-cloud, ✅) · BT-O1–O4 `orun task --epic/--contract`, `orun task epic|milestone`, the 33-tool manifest in `orun mcp serve`, `orun pr open --branch-slug` (orun) · BT1 `track.sh` over `orun task` (idempotent, degrades loudly) · BT2 `land-pr.sh` on `orun/<KEY>-<phase>` + trailer + manifest · BT3 phases ensure milestone + task, umbrella ensures the epic · BT4 phase 01 lands as a PR · BT5 the agent lays out the programme and reports the rollup · BT6 rehearsal + contract test. |
