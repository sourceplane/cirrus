...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Cirrus({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => CirrusError]> = [
  ...
  it("forward-compat: unknown error code → base CirrusError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(CirrusError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as CirrusError).code).toBe("future_quota_exceeded");
    }
  });
