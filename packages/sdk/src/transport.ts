// Base HTTP transport for the Cirrus SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `CirrusError`
...
export interface ClientOptions {
  /**
   * Base URL of the Cirrus api-edge worker, e.g.
   * `https://api.cirrus.app`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `Cirrus` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "Cirrus SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "Cirrus SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
