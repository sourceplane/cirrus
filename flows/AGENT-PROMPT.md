...
subcommand on older CLIs). Install orun v2.56.0+ if missing or older:

```bash
OS=$(uname -s | tr '[:upper:]' '[:lower:]'); ARCH=$(uname -m); [ "$ARCH" = x86_64 ] && ARCH=amd64; [ "$ARCH" = aarch64 ] && ARCH=arm64
curl -fsSL "https://github.com/sourceplane/orun/releases/download/v2.56.0/orun_2.56.0_${OS}_${ARCH}.tar.gz" | tar xz
