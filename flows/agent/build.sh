...
say "UPDATE: starting the $name build into $full (workspace $ws). Expect about 60 minutes; I will report each phase as it finishes."
start="$(date +%s)"
set +e
orun workflow run "$L/flows/phases/00-all/workflow.yaml" \
  ...
  | awk '
    BEGIN {
      ...
      p["03-infrastructure"]="infrastructure is live: the data plane is provisioned on stage and prod"
