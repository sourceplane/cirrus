...
# PROVENANCE, over the BRANDED tree. Deliberately not a search for "cirrus":
...
# is a sentence about where the baseline came from — and unlike BE5a's gate,
# which could only read this repository's copy of the two files, this reads
# what the product ended up with, after every rewrite.
PROVENANCE = re.compile(
    r"\blumen\b|\bfork(ed)? (of|from)\b|\bbaseline (repo|repository|commit)\b", re.I)
...
for ph in 02-foundation 03-infrastructure 04-workers 05-edge 06-console 07-domain; do
