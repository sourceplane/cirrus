...
export const qk = {
  ...
  // Linkfolio
  myPage: () => ["pages", "mine"] as const,
  myBlocks: () => ["pages", "blocks"] as const,
  pageAnalytics: (days: number) => ["pages", "analytics", days] as const,
  publicPage: (handle: string) => ["pages", "public", handle] as const,
};
