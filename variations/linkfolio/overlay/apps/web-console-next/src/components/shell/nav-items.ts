...
export function buildNavSections(scope: NavScope, soloMode: boolean = SOLO_MODE): NavSection[] {
  ...
  // The product section leads under Solo: the creator's own surfaces, none of
...
export function isLinkActive(href: string, pathname: string | null): boolean {
  ...
  // The editor is the product home and its siblings are nested under it, so
  // `/page` must not stay highlighted on `/page/appearance` or `/page/analytics`.
  if (href === "/page") return pathname === "/page";
