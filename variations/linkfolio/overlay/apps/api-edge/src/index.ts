...
import { isPagesRoute, handlePagesRoute } from "./pages-facade";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    } else if (isPagesRoute(url.pathname)) {
      // Linkfolio: the owner's page (/v1/me/page...) and the public page
      // (/v1/p/:handle). Never Solo-suppressed — this IS the single-user
      // product surface.
      response = await handlePagesRoute(request, env, requestId, url.pathname);
