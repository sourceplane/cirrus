...
const LIMITS: Record<RouteFamily, FamilyConfig> = {
  ...
  // Linkfolio: editor writes are per-creator, but the click endpoint is
  // anonymous visitor traffic that shares the fallback bucket — so the org
  // bucket is deliberately generous while the identity bucket stays tight.
  pages: {
    identity: { limit: 60, windowSec: 60 },
