...
import { isHabitsRoute, handleHabitsRoute } from "./habits-facade";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    } else if (isHabitsRoute(url.pathname)) {
      // Streakly: the person's own habits, check-ins and views. Never
      // Solo-suppressed — this IS the single-user product surface.
      response = await handleHabitsRoute(request, env, requestId, url.pathname);
