...
import { Building2, FolderKanban, Gauge, Settings, User2, CheckCircle2, ListChecks, CalendarDays, type LucideIcon } from "lucide-react";
import { SOLO_MODE } from "@/lib/solo-mode";
import { PRODUCT_TABS } from "@/lib/product";
...
export function BottomTabs() {
  ...
  const PRODUCT_ICONS: Record<string, LucideIcon> = { CheckCircle2, ListChecks, CalendarDays, Settings };
  const tabs: Tab[] = SOLO_MODE
    ? PRODUCT_TABS.map((t) => ({ href: t.href, label: t.label, icon: PRODUCT_ICONS[t.icon] ?? Settings }))
    : orgSlug
    ? [
