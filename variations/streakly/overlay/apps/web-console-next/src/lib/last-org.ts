...
import { landingDestination } from "./product";
...
export async function resolvePostAuthDestination(client: PostAuthClient): Promise<string> {
  try {
    ...
    if (serverSlug) {
      ...
      return landingDestination(defaultOrgDestination(serverSlug));
    }
  } catch {
  ...
  try {
    ...
    return landingDestination(defaultOrgDestination(home.slug));
  } catch {
    /* fall back to the local cache */
  }
  return landingDestination(defaultOrgDestination(readLastOrgSlug()));
}
