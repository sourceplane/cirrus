...
export const qk = {
  ...
  // Launchpad
  myLaunches: () => ["launches", "mine"] as const,
  myLaunch: (productId: string) => ["launches", "mine", productId] as const,
  myMakerProfile: () => ["launches", "profile"] as const,
  launchFeed: (range: string) => ["launches", "feed", range] as const,
  launch: (slug: string) => ["launches", "slug", slug] as const,
  launchComments: (slug: string) => ["launches", "comments", slug] as const,
  maker: (handle: string) => ["launches", "maker", handle] as const,
};
