...
import { PRODUCT_LABEL, PRODUCT_NAV } from "../../lib/product";
...
export function buildNavSections(scope: NavScope, soloMode: boolean = SOLO_MODE): NavSection[] {
  ...
  // The product section leads under Solo: the user's own surfaces, none of
  // which carry an organization (see lib/product.ts).
  if (soloMode) {
    sections.push({
      id: "product",
      label: PRODUCT_LABEL,
      links: PRODUCT_NAV.map((l) => ({ href: l.href, label: l.label, icon: l.icon })),
    });
  }
...
export function isLinkActive(href: string, pathname: string | null): boolean {
  ...
  // Product routes with nested "create" pages: keep the parent exact so
  // `/launches` and `/launches/new` never highlight together.
  if (href === "/launches") return pathname === "/launches" || /^\/launches\/(?!new$)[^/]+$/.test(pathname);
