...
const LIMITS: Record<RouteFamily, FamilyConfig> = {
  ...
  // Launchpad: user-scoped writes (submit, vote, comment). No org in the path,
  // so the identity bucket is the one that bites; the org bucket keys off the
  // anonymous fallback and stays generous.
  launches: {
    identity: { limit: 60, windowSec: 60 },
    org: { limit: 600, windowSec: 60 },
  },
