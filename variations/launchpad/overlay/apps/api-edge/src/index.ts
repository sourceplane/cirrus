...
import { isLaunchesRoute, handleLaunchesRoute } from "./launches-facade";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    } else if (isLaunchesRoute(url.pathname)) {
      // Launchpad: user-scoped owner routes (/v1/me/...) and the public
      // directory (/v1/launches*, /v1/makers/*). Never Solo-suppressed —
      // this IS the single-user product surface.
      response = await handleLaunchesRoute(request, env, requestId, url.pathname);
