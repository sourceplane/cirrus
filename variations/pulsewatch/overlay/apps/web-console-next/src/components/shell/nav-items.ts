...
export function buildNavSections(scope: NavScope, soloMode: boolean = SOLO_MODE): NavSection[] {
  ...
  // The product section leads under Solo: the developer's own surfaces, none
  // of which carry an organization (see lib/product.ts).
  if (soloMode) {
    sections.push({
      id: "product",
      label: PRODUCT_LABEL,
...
export function isLinkActive(href: string, pathname: string | null): boolean {
  ...
  if (href === "/monitors") return pathname === "/monitors" || pathname.startsWith("/monitors/");
