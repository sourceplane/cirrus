...
import { isMonitorsRoute, handleMonitorsRoute } from "./monitors-facade";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    } else if (isMonitorsRoute(url.pathname)) {
      // Pulsewatch: the owner's monitors (/v1/me/...) and the public status
      // page (/v1/status/:handle). Never Solo-suppressed — this IS the
      // single-user product surface.
      response = await handleMonitorsRoute(request, env, requestId, url.pathname);
