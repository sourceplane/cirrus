...
export function buildNavSections(scope: NavScope, soloMode: boolean = SOLO_MODE): NavSection[] {
  ...
  // The product section leads under Solo: the person's own surfaces, none of
...
export function isLinkActive(href: string, pathname: string | null): boolean {
  ...
  // The list has nested detail routes; the other product links do not.
  if (href === "/subscriptions") return pathname === "/subscriptions" || pathname.startsWith("/subscriptions/");
