...
import { isSubscriptionsRoute, handleSubscriptionsRoute } from "./subscriptions-facade";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    } else if (isSubscriptionsRoute(url.pathname)) {
      // Subtally: the person's own subscriptions and their totals. Never
      // Solo-suppressed — this IS the single-user product surface.
      response = await handleSubscriptionsRoute(request, env, requestId, url.pathname);
