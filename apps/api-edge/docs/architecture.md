...
- **Service bindings** → `projects-worker`, `events-worker`, `config-worker`, `webhooks-worker`, `billing-worker`, `integrations-worker`, `identity-worker`, `membership-worker`, `metering-worker`, `notifications-worker` —
...
- **Wired configuration** (resolved at deploy time from job-output
  ...
  `WIRING_CLOUDFLARE_D1_STAGE`, `WIRING_CLOUDFLARE_D1_PROD`, `WIRING_CLOUDFLARE_KV_STAGE`, `WIRING_CLOUDFLARE_KV_PROD`.
