...
Your backend holds one Cirrus API key and **zero GitHub credentials**.
...
const cirrus = new Cirrus({
  baseUrl: process.env.CIRRUS_API_URL!,
  token: process.env.CIRRUS_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await cirrus.integrations.issueGithubToken(ORG_ID, {
