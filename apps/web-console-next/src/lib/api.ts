...
// deleted; all wire I/O now goes through the typed `Cirrus` resource
...
//   - `createClient` — constructs a `Cirrus` against a target, wired
...
//     `Promise<T>` + thrown `CirrusError` into the same shape the
...
import { Cirrus, CirrusError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Cirrus;
...
export function createClient(target: ApiTarget, token: string | null): Cirrus {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Cirrus(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `CirrusError` subclasses surface as `{ ok: false }` carrying the
