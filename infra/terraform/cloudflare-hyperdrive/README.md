...
- **`cloudflare_hyperdrive_config`**: Hyperdrive gateway resource
  - **Name**: `{namespacePrefix}cirrus-{environment}` (e.g., `cirrus-stage`)
...
| `orgName` | string | `cirrus` | Organization name (for tags and secret paths) |
| `owner` | string | `cirrus` | Repository owner (for tags) |
| `repo` | string | `cirrus` | Repository name (for tags and secret paths) |
| `namespace` | string | `cirrus` | Namespace (for labels and naming conventions) |
...
1. **`supabase` component** (same repo)
   ...
   - This component reads credentials from the secret path: `sourceplane/cirrus/supabase/{environment}`
...
- **Secret path**: `sourceplane/cirrus/supabase/{environment}`
...
aws secretsmanager get-secret-value --secret-id sourceplane/cirrus/supabase/stage
```
